...
func TestAdjustForFilename(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expected, actions.AdjustForFilename(tc.s, tc.length))
		})
	}
}
...
func TestFormatTime(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.expected, actions.FormatTime(tt))
		})
	}
}
...
func TestFormatDuration(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tc.expected, actions.FormatDuration(tc.dur))
		})
	}
}

func TestBuildOutputStem(t *testing.T) {
	t.Parallel()
	ctx := &actions.LocateOutputContext{
		Title:          "En direct : Titre de la   vidéo — 24h/7 | Panorama, 360 / ? ",
		ID:             "abc123",
		InputStartTime: time.Date(2026, 1, 2, 10, 20, 30, 0, time.UTC),
		InputDuration:  2*time.Minute + 3*time.Second,
	}
	assert.Equal(
		t,
		"En-direct-Titre-de-la-video_abc123_20260102T102030+00_2m3s",
		actions.BuildOutputStem(ctx),
	)
}
